...
def save_checkpoint_best_map_0_5(save_dir):
    filename = osp.join(save_dir, 'last_ckpt.pt')
    best_map_0_5_filename = osp.join(save_dir, 'best_map_0_5_ckpt.pt')
    shutil.copyfile(filename, best_map_0_5_filename)
...
def strip_optimizer_gdc(ckpt_dir, epoch):
    """Delete optimizer from saved checkpoint file"""
    for s in ['best', 'last']:
        ckpt_path = osp.join(ckpt_dir, '{}_ckpt.pt'.format(s))
        if not osp.exists(ckpt_path):
        ...
        # delete gdc
        if ckpt.get('ema_gdc'):
            ckpt['gdc_net'] = ckpt['ema_gdc']
        for k in ['optimizer_gdc', 'ema_gdc', 'updates_gdc']:
            ckpt[k] = None
        ckpt['epoch'] = epoch
        ckpt['model'].half()  # to FP16
        ckpt['gdc_net'].half()
        for p in ckpt['model'].parameters():
            p.requires_grad = False
        for p in ckpt['gdc_net'].parameters():
            p.requires_grad = False
        torch.save(ckpt, ckpt_path)
