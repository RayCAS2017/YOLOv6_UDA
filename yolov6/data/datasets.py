    ...
    def get_imgs_labels(self, img_dir):
        ...
        if self.task.lower() == "val":
            if self.data_dict.get("is_coco", False): # use original json file when evaluating on coco dataset.
                for ikey in self.data_dict.keys():
                    if 'anno_path' in ikey:
                        assert osp.exists(self.data_dict[ikey]), "Eval on coco dataset must provide valid path of the annotation file in config file: data/coco.yaml"
            else:
                assert (
                    self.class_names
