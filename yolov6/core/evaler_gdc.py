...
class Evaler:
    ...
    def init_data(self, dataloader, task):
        ...
        if task != 'train':
            ...
            if self.domain == 'source':
                dataloader = \
                create_dataloader(self.data[task + '_s' if task in ('train', 'val', 'test') else 'val_s'],
                                  self.img_size, self.batch_size, self.stride, hyp=eval_hyp, check_labels=True, pad=pad,
                                  rect=rect, data_dict=self.data, task=task)[0]
            elif self.domain == 'target':
                dataloader = \
                    create_dataloader(self.data[task + '_t' if task in ('train', 'val', 'test') else 'val_t'],
                                      self.img_size, self.batch_size, self.stride, hyp=eval_hyp, check_labels=True,
                                      pad=pad, rect=rect, data_dict=self.data, task=task)[0]
    ...
    def predict_model(self, model, dataloader, task):
        ...
        pred_results = []
        if self.domain == 'source':
            pbar = tqdm(dataloader, desc="Inferencing model in source val datasets.", ncols=NCOLS)
        elif self.domain == 'target':
            pbar = tqdm(dataloader, desc="Inferencing model in target val datasets.", ncols=NCOLS)
        else:
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                csv_file = os.path.join(self.save_dir, "class_pr_and_map.csv")
                if os.path.exists(csv_file):
                    csv_file = os.path.join(self.save_dir, "class_pr_and_map_1.csv")
                csv_head = ['Class', 'Images', 'Labels', 'P@.5iou', 'R@.5iou', 'F1@.5iou', 'mAP@.5', 'mAP@.5:.95']
                csv_data = []
                csv_data.append(['all', seen, nt.sum(), round(mp, 3), round(mr, 3), round(f1.mean(0)[AP50_F1_max_idx], 3),round(map50, 3), round(map, 3)])

                # self.pr_metric_result = (map50, map, mp, mr, f1.mean(0)[AP50_F1_max_idx])
                self.pr_metric_result = (map50, map)

                pr_results.append([mp, mr, f1.mean(0)[AP50_F1_max_idx]])
                ...
                if self.verbose and model.nc > 1:
                    for i, c in enumerate(ap_class):
                        ...
                        csv_data.append([model.names[c], seen, nt[c], round(p[i, AP50_F1_max_idx], 3),
                                         round(r[i, AP50_F1_max_idx], 3),
                                         round(f1[i, AP50_F1_max_idx], 3), round(ap50[i], 3), round(ap[i], 3)])
                ...
                pr_results.append([0.0, 0.0, 0.0])
        
        if self.do_pr_metric:
            return pred_results, vis_outputs, vis_paths, pr_results[0]
        else:
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != 'speed' and len(pred_results):
            if self.domain=='source' and 'anno_path_s' in self.data:
                anno_json= self.data['anno_path_s']
            elif self.domain=='source' and 'anno_path_s' not in self.data:
                dataset_root = os.path.dirname(os.path.dirname(self.data['val_s']))
                base_name = os.path.basename(self.data['val_s'])
                anno_json = os.path.join(dataset_root, 'annotations', f'instances_{base_name}.json')
            elif self.domain=='target' and 'anno_path_t' in self.data:
                anno_json = self.data['anno_path_t']
            elif self.domain=='target' and 'anno_path_t' not in self.data:
                dataset_root = os.path.dirname(os.path.dirname(self.data['val_t']))
                base_name = os.path.basename(self.data['val_t'])
                anno_json = os.path.join(dataset_root, 'annotations', f'instances_{base_name}.json')

            if self.domain == 'source':
                pred_json = os.path.join(self.save_dir, "predictions_source.json")
            elif self.domain == 'target':
                pred_json = os.path.join(self.save_dir, "predictions_target.json")
            ...
            if self.verbose:
                ...
                for _, ann_i in cocoEval.cocoGt.anns.items():
                    ...
                    nc_i = self.ids.index(ann_i['category_id'])
    ...
    def reload_dataset(data):
        ...
        val_s = data.get('val_s')
        val_t = data.get('val_t')
        if not os.path.exists(val_s):
            raise Exception('Dataset val_s not found.')
        if not os.path.exists(val_t):
            raise Exception('Dataset val_t not found.')
        return data
    ...
    def eval_trt(self, engine, stride=32):
        ...
        def init_data(dataloader, task):
            ...
            dataloader = create_dataloader(self.data[task if task in ('train', 'val', 'test') else 'val'],
                                               ...
                                               rect=False, data_dict=self.data, task=task)[0]
