...
class Model(nn.Module):
    ...
    def __init__(self, config, channels=3, num_classes=None, anchors=None):  # model, input channels, number of classes
        ...
        if config.get('gdc', False) and config.gdc.get('featuremaps', False):
            self.output_fs = config.gdc.featuremaps
        else:
            self.output_fs = 'backbone'
        # Init weights
    ...
    def forward(self, x):
        ...
        x = self.backbone(x)
        if self.output_fs == 'backbone':
            bf = x
        # bf = x
        x = self.neck(x)
        if self.output_fs == 'fpn':
            bf = x
        if export_mode == False:
            featmaps = []
            featmaps.extend(x)
        x = self.detect(x)
        return x if export_mode is True else [x, featmaps, bf]
