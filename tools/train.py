...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--gdc', action='store_true', help='global domain classifier or not')
    parser.add_argument('--gdc_eval_domain', default='both', type=str, help='source, target, or both, eval the valid dataset of the correspond domain')
    parser.add_argument('--save_best_ap_0_5', action='store_true', help='save the bset mAp0.5 model')
    parser.add_argument('--do_pr_metric', action='store_true', help='do pr metric')
    parser.add_argument('--mix_train_with_gdc', action='store_true', help='uda train with gdc, mix source with target data, both of which have labels')
    parser.add_argument('--mix_train_wo_gdc', action='store_true', help='uda train without gdc, mix source with target data, both of which have labels')
    return parser
    ...
    if args.resume:
        ...
        if master_process:
            ...
            if os.path.exists(args.data_path):
                data_path_copy = os.path.join(args.save_dir, args.data_path.split('/')[-1])
                shutil.copyfile(os.path.abspath(args.data_path), os.path.abspath(data_path_copy))
            if os.path.exists(args.conf_file):
                conf_file_path_copy = os.path.join(args.save_dir, args.conf_file.split('/')[-1])
                shutil.copyfile(os.path.abspath(args.conf_file), os.path.abspath(conf_file_path_copy))
